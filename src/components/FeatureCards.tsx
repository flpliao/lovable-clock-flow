...
const FeatureCards = ({ abnormalCount, annualLeaveBalance }: FeatureCardsProps) => {
  ...
  const mainFeatures = [
    {
      ...
      gradient: 'from-green-400/25 to-emerald-600/25',
    ...
    },
    {
      ...
      gradient: 'from-orange-400/25 to-orange-600/25',
  ...
  const adminFeatures = [
    ...
    },
    {
      ...
      gradient: 'from-gray-400/25 to-gray-600/25',
  ...
  const FeatureCard = ({ feature }: { feature: any }) => (
    <Card className="group hover:scale-[1.02] transition-all duration-300 border-0 shadow-2xl overflow-hidden backdrop-blur-xl bg-white/15 hover:bg-white/20 border border-white/30">
          ...
          <div className={`bg-gradient-to-br ${feature.gradient} transition-all duration-300 p-5 sm:p-6 relative overflow-hidden`}>
            ...
            <div className="relative z-10">
              <div className="flex items-center justify-between mb-4">
                <div className={`p-3 ${feature.iconBg} rounded-2xl backdrop-blur-xl border border-white/40 shadow-lg`}>
                  <feature.icon className={`h-6 w-6 sm:h-7 sm:w-7 ${feature.iconColor} drop-shadow-md`} />
                </div>
                {feature.badge && (
                  <Badge className="bg-white/30 text-white border-white/40 text-xs backdrop-blur-xl font-medium drop-shadow-md">
                    {feature.badge}
                  </Badge>
                )}
              </div>
              
              <h3 className="font-semibold text-white text-base sm:text-lg mb-2 drop-shadow-lg">
                {feature.title}
              </h3>
  ...
  return (
    <div className="space-y-8">
      {/* 主要功能區 */}
      <div>
        <h2 className="text-xl font-semibold text-white mb-6 text-center sm:text-left drop-shadow-lg">主要功能</h2>
      ...
      {(isAdmin() || isManager()) && (
        <div>
          <h2 className="text-xl font-semibold text-white mb-6 text-center sm:text-left drop-shadow-lg">管理功能</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
