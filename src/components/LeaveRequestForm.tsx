...
export function LeaveRequestForm({ onSubmit }: LeaveRequestFormProps) {
  ...
  async function handleSubmit(data: LeaveFormValues) {
    ...
    console.log('前端選擇的日期:', {
      ...
      start_formatted: formatDateForDatabase(data.start_date),
      end_formatted: formatDateForDatabase(data.end_date)
    });

    // Create the leave request - 現在資料庫使用 Asia/Taipei 時區，日期處理更加直接
    const leaveRequest = {
      id: '', // Will be generated by database
      user_id: currentUser.id,
      start_date: formatDateForDatabase(data.start_date),
      end_date: formatDateForDatabase(data.end_date),
    ...
    console.log('即將提交的請假申請 (資料庫時區: Asia/Taipei):', leaveRequest);
    ...
    <div className="space-y-6">
      <div className="text-center mb-6">
        ...
        <p className="text-white/80 font-medium drop-shadow-sm">
          請填寫以下資訊提交您的請假申請（系統時區：Asia/Taipei +08:00）
        </p>
      </div>
