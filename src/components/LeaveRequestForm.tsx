...
export function LeaveRequestForm({ onSubmit }: LeaveRequestFormProps) {
  ...
  async function handleSubmit(data: LeaveFormValues) {
    ...
    // 使用本地日期，避免時區轉換問題
    const localStartDate = formatDateForDatabase(data.start_date);
    const localEndDate = formatDateForDatabase(data.end_date);

    console.log('瀏覽器本地日期:', {
      selected_start_date: data.start_date,
      selected_end_date: data.end_date,
      formatted_start_date: localStartDate,
      formatted_end_date: localEndDate,
      user_timezone: Intl.DateTimeFormat().resolvedOptions().timeZone
    });

    // Create the leave request - 使用本地日期格式，與資料庫時區 Asia/Taipei 同步
    const leaveRequest = {
    ...
    console.log('即將提交的請假申請（使用本地日期）:', leaveRequest);
