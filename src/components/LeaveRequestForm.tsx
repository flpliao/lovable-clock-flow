...
import { formatDateForDatabase, ensureLocalDate } from '@/utils/dateUtils';
...
export function LeaveRequestForm({ onSubmit }: LeaveRequestFormProps) {
  ...
  async function handleSubmit(data: LeaveFormValues) {
    ...
    // 確保使用本地日期，完全避免時區轉換
    const ensuredStartDate = ensureLocalDate(data.start_date);
    const ensuredEndDate = ensureLocalDate(data.end_date);
    
    const localStartDate = formatDateForDatabase(ensuredStartDate);
    const localEndDate = formatDateForDatabase(ensuredEndDate);

    console.log('請假申請日期處理詳細日誌:', {
      form_start_date: data.start_date,
      form_end_date: data.end_date,
      ensured_start_date: ensuredStartDate,
      ensured_end_date: ensuredEndDate,
      formatted_start_date: localStartDate,
      formatted_end_date: localEndDate,
      user_timezone: Intl.DateTimeFormat().resolvedOptions().timeZone,
      current_time: new Date().toString(),
      form_data_raw: data
    });

    // Create the leave request - 使用確保本地化的日期格式
    const leaveRequest = {
    ...
    console.log('即將提交的請假申請（確保本地日期）:', leaveRequest);
