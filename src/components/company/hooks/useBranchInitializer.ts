...
export const useBranchInitializer = () => {
  ...
  const initializeDefaultBranch = async () => {
    try {
      ...
      if (checkError) {
        console.error('檢查單位錯誤:', checkError);
        return;
      }

      if (existingBranch) {
        console.log('預設單位已存在:', existingBranch);
        return;
      }

      // 如果不存在，則新增預設總公司
      console.log('新增預設總公司單位...');
      ...
      if (error) {
        console.error('新增預設單位失敗:', error);
        if (error.message.includes('duplicate key')) {
          console.log('單位已存在，跳過新增');
          return;
        }
        throw error;
      }

      console.log('成功新增預設單位:', data);
    } catch (error) {
      console.error('初始化預設單位失敗:', error);
      // 靜默處理錯誤
    }
  };
