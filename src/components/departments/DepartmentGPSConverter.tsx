...
const DepartmentGPSConverter: React.FC<DepartmentGPSConverterProps> = ({
  ...
  return <div className="space-y-4">
      ...
      <div className="space-y-2">
        ...
        <div className="space-y-2">
          <Input id={`address-${department.id}`} value={address} onChange={e => handleAddressChange(e.target.value)} placeholder="請輸入完整地址（如：台南市東區長榮路一段85號）" className={`text-sm ${validation && !validation.isValid ? 'border-red-300 focus:border-red-500' : ''}`} disabled={loading} />
        ...
        {/* 地址格式提示 */}
        {validation && !validation.isValid && <div className="flex items-start gap-2 p-2 bg-red-50 border border-red-200 rounded text-xs">
            <AlertCircle className="h-3 w-3 text-red-500 mt-0.5 flex-shrink-0" />
            <div className="space-y-1">
              <div className="text-red-700 font-medium">地址格式需要改進：</div>
              {validation.errors.map((error, index) => <div key={index} className="text-red-600">• {error}</div>)}
              {validation.suggestions.length > 0 && <div className="text-red-600 mt-1">
                  <div className="font-medium">建議格式：</div>
                  {validation.suggestions.slice(0, 3).map((suggestion, index) => <div key={index} className="ml-2">• {suggestion}</div>)}
                </div>}
            </div>
          </div>}
        
        {/* 格式正確提示 */}
        {validation && validation.isValid && <div className="flex items-center gap-2 text-xs text-green-600">
            <CheckCircle2 className="h-3 w-3" />
            <span>地址格式正確</span>
          </div>}
        
        {/* 地址建議列表 */}
        {showSuggestions && addressSuggestions.length > 0 && <Card className="mt-2">
            <CardContent className="p-3">
              <div className="text-xs font-medium mb-2 text-gray-700">建議地址：</div>
              <div className="space-y-1 max-h-32 overflow-y-auto">
                {addressSuggestions.slice(0, 5).map((suggestion, index) => <button key={index} onClick={() => handleSelectSuggestion(suggestion)} className="w-full text-left p-2 text-xs hover:bg-gray-50 rounded border border-gray-200 transition-colors">
      ...
      <Button onClick={handleConvert} disabled={loading || !address.trim() || validation && !validation.isValid} size="sm" className="w-full">
        {loading ? <>
      ...
      <div className="text-xs text-gray-600 bg-gray-50 p-2 rounded space-y-2">
        ...
        {department.gps_status === 'failed' && <div className="bg-yellow-50 border border-yellow-200 p-2 rounded mt-2">
