...
export const AnnualLeaveInfoCard = React.memo(function AnnualLeaveInfoCard({ 
  ...
  if (!staffData.hire_date) {
    return (
      <div className="bg-orange-50/90 backdrop-blur-sm border border-orange-200/50 rounded-2xl shadow-sm p-6">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            ...
            <p className="text-sm text-orange-700 font-medium">Annual Leave Setup Required</p>
          </div>
        </div>
        <div className="bg-orange-100/80 border border-orange-200 rounded-xl p-4">
  ...
  return (
    <div className="bg-white/90 backdrop-blur-sm border border-gray-200/50 rounded-2xl shadow-sm p-6 hover:shadow-md transition-shadow">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h3 className="text-lg font-semibold text-gray-900">特別休假資訊</h3>
          <p className="text-sm text-gray-500 font-medium">Annual Leave Information</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        ...
        <div className="bg-gradient-to-br from-blue-50 to-blue-100 border border-blue-200 rounded-xl p-4 text-center">
          <div className="text-3xl font-bold text-blue-700 mb-1">{staffData.totalAnnualLeaveDays}</div>
          <div className="text-blue-600 text-sm font-medium">年度總額（天）</div>
          <div className="text-blue-500 text-xs mt-1">{staffData.totalAnnualLeaveDays * 8} 小時</div>
        </div>

        {/* 已使用天數 */}
        <div className="bg-gradient-to-br from-orange-50 to-red-50 border border-orange-200 rounded-xl p-4 text-center">
          <div className="text-3xl font-bold text-orange-700 mb-1">{staffData.usedAnnualLeaveDays}</div>
          <div className="text-orange-600 text-sm font-medium">已使用（天）</div>
          <div className="text-orange-500 text-xs mt-1">{staffData.usedAnnualLeaveDays * 8} 小時</div>
        </div>

        {/* 剩餘天數 */}
        <div className="bg-gradient-to-br from-green-50 to-emerald-50 border border-green-200 rounded-xl p-4 text-center">
          <div className="text-3xl font-bold text-green-700 mb-1">{staffData.remainingAnnualLeaveDays}</div>
          <div className="text-green-600 text-sm font-medium">剩餘可用（天）</div>
          <div className="text-green-500 text-xs mt-1">{staffData.remainingAnnualLeaveDays * 8} 小時</div>
        </div>
      </div>
      ...
      <div className="bg-gray-50/80 border border-gray-100 rounded-xl p-4">
        <div className="flex items-center justify-between mb-3">
          ...
          <span className="text-gray-900 font-bold text-lg">{usagePercentage}%</span>
        </div>
