...
export function NewLeaveRequestForm({ onSubmit }: NewLeaveRequestFormProps) {
  ...
  const calculatedHours = React.useMemo(() => {
    if (watchedValues.start_date && watchedValues.end_date) {
      return calculateWorkHours(watchedValues.start_date, watchedValues.end_date);
    }
    return 0;
  ...
  const selectedLeaveType = React.useMemo(() => {
    if (!watchedValues.leave_type) return null;
    return leaveTypes.find(type => type.code === watchedValues.leave_type) || null;
  }, [watchedValues.leave_type, leaveTypes]);
