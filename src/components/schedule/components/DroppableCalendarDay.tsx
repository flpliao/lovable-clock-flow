...
const DroppableCalendarDay = ({
  ...
  if (!day || !day.date) {
    return <div className="min-h-[100px] border-r border-white/15 last:border-r-0"></div>;
  }
  ...
  return <div ref={setNodeRef} onClick={handleCellClick} className={`
        min-h-[100px] p-3 border-r border-white/15 last:border-r-0 
        cursor-pointer transition-all duration-200
        ${isExtendedMonth ? 'bg-gray-100/10' : ''}
        ${isToday ? 'bg-gradient-to-br from-blue-400/25 to-blue-300/15 shadow-inner border-blue-300/30' : ''}
        ${isOver ? 'bg-gradient-to-br from-green-400/30 to-green-300/20 ring-2 ring-green-400/50 shadow-lg' : 'hover:bg-white/10 hover:shadow-md'}
      `}>
      <div className="flex flex-col h-full">
        {/* 日期標題 */}
        <div className={`text-sm mb-1 font-semibold drop-shadow-sm ${isCurrentMonth ? day.isWeekend ? 'text-red-600' : 'text-gray-800' : 'text-gray-500'} ${isToday ? 'font-bold text-blue-700' : ''}`}>
          {day.label}
        </div>
        ...
        {/* 排班列表 */}
        <div className="flex-1 space-y-1 bg-cyan-100">
          ...
          {/* 顯示更多排班提示 */}
          {daySchedules.length > 3 && <div className="text-xs text-gray-700 bg-white/40 px-2 py-1 rounded-lg text-center cursor-pointer hover:bg-white/50 transition-all duration-200 shadow-sm border border-white/30 backdrop-blur-sm">
              還有 {daySchedules.length - 3} 個排班...
            </div>}

          {/* 拖拽提示區域 */}
          {isOver && daySchedules.length === 0 && <div className="text-xs text-green-800 bg-green-200/50 px-2 py-1 rounded-lg text-center border-2 border-dashed border-green-400/50 shadow-sm backdrop-blur-sm">
              放開以移動排班到這天
            </div>}
        </div>
      </div>
