    ...
    >
      <div className="flex flex-col h-full">
        ...
        <div className={`text-sm mb-1 font-semibold drop-shadow-sm ${
          isCurrentMonth ? (day.isWeekend ? 'text-red-600' : 'text-gray-800') : 'text-gray-500'
        ...
        <div className="flex-1 space-y-1">
          ...
          {daySchedules.length > 3 && (
            <div className="text-xs text-gray-700 bg-white/40 px-2 py-1 rounded-lg text-center cursor-pointer hover:bg-white/50 transition-all duration-200 shadow-sm border border-white/30 backdrop-blur-sm">
          ...
          {isOver && daySchedules.length === 0 && (
            <div className="text-xs text-green-800 bg-green-200/50 px-2 py-1 rounded-lg text-center border-2 border-dashed border-green-400/50 shadow-sm backdrop-blur-sm">
              放開以移動排班到這天
            </div>
          )}
        </div>
