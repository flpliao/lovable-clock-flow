...
export const useSupabaseCheckIn = () => {
  ...
  const createCheckInRecord = useCallback(
    async (record: Omit<CheckInRecord, 'id'>) => {
      if (!employee?.slug) {
      ...
      try {
        // 先驗證用戶是否存在於 staff 表中
        const staffId = await validateAndGetStaffId(employee.slug);
        ...
        const success = await createRecord(record, employee.slug);
        if (success) {
          // 重新載入記錄
          await refreshData();
        }
    ...
    [employee?.slug, createRecord, validateAndGetStaffId]
  );

  const getTodayRecords = useCallback(
    async (userId?: string) => {
      const targetUserId = userId || employee?.slug;
      if (!targetUserId) {
    ...
    [employee?.slug, getTodayCheckInRecords, validateAndGetStaffId]
  );
  ...
  const refreshData = useCallback(async () => {
    if (employee?.slug) {
      try {
        ...
        console.log('重新整理打卡資料，使用者 ID:', employee.slug);
        await loadCheckInRecords(employee.slug);
      } catch (error) {
        console.error('重新整理資料失敗:', error);
        // 不顯示用戶錯誤提示，只記錄到控制台
      }
    }
  }, [employee?.slug, loadCheckInRecords, validateAndGetStaffId]);

  // 手動載入記錄 - 加強錯誤處理
  const manualLoadRecords = useCallback(async () => {
    if (employee?.slug && !loading && !initialLoadRef.current) {
      try {
        // 先驗證用戶
        const staffId = await validateAndGetStaffId(employee.slug);
        if (!staffId) {
        ...
        console.log('手動載入打卡記錄，使用者ID:', employee.slug);
  ...
  }, [employee?.slug, loadCheckInRecords, loading, validateAndGetStaffId]);
