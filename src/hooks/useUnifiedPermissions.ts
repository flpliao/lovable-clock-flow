...
interface UseUnifiedPermissionsProps {
  staffList: Staff[];
}

export const useUnifiedPermissions = ({ staffList }: UseUnifiedPermissionsProps) => {
  const currentUser = useCurrentUser();
  const [backendRoles, setBackendRoles] = useState<StaffRole[]>([]);
  const [rolesLoading, setRolesLoading] = useState(true);

  const permissionService = useMemo(() => UnifiedPermissionService.getInstance(), []);
