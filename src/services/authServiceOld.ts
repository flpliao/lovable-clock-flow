...
export class AuthService {
  ...
  static async authenticate(
    email: string,
    password: string
  ): Promise<{ success: boolean; user?: AuthUser; error?: string; session?: unknown }> {
    try {
  ...
  static buildUserFromStaff(authUser: SupabaseUser, staffData: Record<string, unknown>): AuthUser {
    const user: AuthUser = {
      ...
      onboard_date:
        typeof staffData.onboard_date === 'string'
          ? staffData.onboard_date
  ...
  static createFallbackUser(authUser: SupabaseUser, email: string): AuthUser {
    let userName = '';
    if (
      typeof (authUser as { user_metadata?: unknown }).user_metadata === 'object' &&
      authUser &&
      (authUser as { user_metadata?: { name?: unknown } }).user_metadata?.name &&
      typeof (authUser as { user_metadata?: { name?: unknown } }).user_metadata?.name === 'string'
    ) {
  ...
  static async createStaffRecord(authUser: SupabaseUser, email: string): Promise<void> {
    try {
      ...
      if (
        ...
        userName = (authUser as { user_metadata?: { name?: unknown } }).user_metadata
          ?.name as string;
      } else if (authUser.email && typeof authUser.email === 'string') {
        userName = authUser.email.split('@')[0];
      } else {
      ...
      const { error: insertError } = await supabase.from('staff').insert([newStaffData]);
